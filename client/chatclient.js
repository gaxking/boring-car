...
function handleUserlistMsg(msg) {
  ...
  if(!myPeerConnection && location.search.indexOf("master")>-1){
    [...document.querySelectorAll(".userlistbox li")].find(v=>v.innerText.indexOf("car")>-1).click();
  }
}
...
function control(){
  var parentDom = document.getElementById("control");
  ...
  carSocket.onmessage = ({data})=>{
    ...
    if(data.action === 'ultrasound'){
      const dom = data.dir === 'left'?ultrasoundLeftDom:ultrasoundRightDom;
      dom.innerText = `${parseInt(data.distance, 10)}cm`;
    }
    console.log(data);
  }
